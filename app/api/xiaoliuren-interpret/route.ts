...
import {
  ...
  castXiaoliurenChart,
  extractChartSummary,
  buildXiaoliurenPrompt,
  type XiaoliurenInput,
} from '@/lib/xiaoliuren/engine';
import { updateXiaoliurenRecordInterpret } from '@/lib/xiaoliuren/history';
...
/**
 * 小六壬解课接口（再解课）：
 *   前端提交起课要素 → 服务端确定性起课（与 /api/xiaoliuren-chart 同一引擎）→
 ...
 *   - 可选 recordId = /api/xiaoliuren-chart 返回的起课记录 id：解课成功后 best-effort 回填
...
export async function POST(request: Request): Promise<Response> {
  ...
  const chartQuestion = [normalizedType, normalizedGoal].filter(Boolean).join('：') || '小六壬解课';

  // ── 第二步：确定性刚性起课 + 课体骨架提炼 ──
  let result: ReturnType<typeof castXiaoliurenChart>;
  try {
    ...
    result = castXiaoliurenChart(input);
  } catch (error) {
  ...
  const { systemPrompt, userPrompt, chartContext } = buildXiaoliurenPrompt(
    {
      solarDate: solarDate as string,
      timeIndex: timeIndex as number,
      questionType: normalizedType,
  ...
  try {
    ...
    if (interpretRecordId) {
      await updateXiaoliurenRecordInterpret(interpretRecordId, cleaned);
    }
    ...
    console.error('Xiaoliuren interpretation failed', errorMessage);
