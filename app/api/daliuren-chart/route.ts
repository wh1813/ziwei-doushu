...
import { saveDaliurenRecord } from '@/lib/daliuren/history';
...
/**
 * 大六壬起课接口（先起课）：
 *   前端提交起课日期 + 时辰 + 问事 → 服务端确定性起课（天地盘/四课/三传/六亲/天乙）
 *   → 返回结构化课体 JSON；解课另行调用 /api/daliuren-interpret。
 *
 * 请求体：{
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const result = castDaliurenChart(input);
    // ── 第三步：起课历史落库（best-effort：D1 不可用/表未建时静默跳过，不影响起课）──
    const recordId = await saveDaliurenRecord(result);
