...
3. 排盘完成后，定位"代表个人的符号"时才使用出生八字（系统已预计算，见【个人用神定位】，若提供）：
   - 婚恋/情感调理：本人符号 = 出生日干（日干为甲者以出生旬首遁仪代理）；日干合神（五合之配）= 日干的天干五合之干（甲己合、乙庚合、丙辛合、丁壬合、戊癸合），古法以五合论夫妇之配，解盘时须用「日干合神」这一规范术语，不得使用"灵魂伴侣"等口语化叫法。
...
2. 用神取用顺序（先主后辅，不堆砌）：
   ...
   - 感情：男问女取乙，女问男取庚，同性关系取六合；若提供个人用神，须以"本人=出生日干、日干合神=日干五合干"为主轴解读。
...
【调理建议操作性铁律（每条建议必须让人能直接照做）】
1. 物品建议必须紧跟实例：凡输出"移除/清理/放置某类物品"，必须以"如"字列举3至5个常见、具体、易得的实物，严禁只写抽象类别就结束。示例映射（按体系意象给实物，可结合宫位五行与星门神属性延展，实例必须是现实中买得到、找得到的东西）：
...
3. 层级只用纯文本表达：段落标题以【】单独成行（与下方八段结构的段名完全一致）；段内分点用「1. 2. 3.」编号；强调用「」括注，不用任何符号修饰。
...
第二步·个人用神定位：${hasPersonal ? '系统已按出生八字锁定「本人=出生日干、日干合神=日干五合干（五合古法取夫妇之配）、生年天干（甲以值符代甲并以遁仪戊为盘面代理）」，直接在九宫格中找到这些天干符号所在的宫位与方位，核查其平安/击刑/入墓/空亡状态' : '未提供出生信息，本步跳过，不得做任何个人层面的推断'}；
...
请严格按以下八段结构化输出（段落标题以【】单独成行、与下列段名完全一致；全文纯文本，禁用一切 Markdown 符号；第一行直接输出第一段标题，不得有任何开场白与客套说明）：
...
4. 【用神与关键依据】明确本次主用神取什么、为何这样取、落在哪一宫；该宫的门、星、神是帮它还是压它。${hasPersonal ? '须在九宫格中逐一指认本人/日干合神/生年天干符号所在宫位与方位，并引用预计算的击刑/入墓/空亡状态；提及合神时用「日干合神」规范术语。' : ''}
5. 【核心判断】对所问事项给出直接研判：成与不成、快与慢、利与不利；必须引用盘面证据${hasPersonal ? '与个人用神预计算事实' : ''}，禁止脱离盘面空谈。
6. 【方位时机建议】给出有利方位与行动时机建议（结合驿马、空亡、吉门所在宫位方向）；方位必须指明具体空间场景（主卧/客厅/办公工位）与指南针定位方法，不得只写"西方方位"。
7. 【解局与调理建议】对照预计算的不利清单（击刑/入墓/空亡/烂桃花小人/孤辰寡宿），按"先灭象、再布阵"逐条给出可执行的调理操作与禁忌；每条物品类建议必须以"如"字列举3至5个具体实物，每条方位类建议必须写明房间场景与定位方法；若全部平安，明确说明本局无需调理动作。
