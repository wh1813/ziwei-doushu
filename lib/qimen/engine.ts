...
1. 【已确认信息】复述所问事项与起局时间（公历日期、时辰、干支）${hasPersonal ? '；一并复述出生信息与八字日柱、生年干支' : ''}。
2. 【使用规则】说明本盘阴阳遁、局数、三元、旬首；若涉中宫寄坤或节气边界，此处明确提示；点明"起局用当前时间、定位个人符号才用出生八字"的口径。
3. 【盘面摘要】概述值符、值使、时空亡与日空亡、驿马与关键宫位要素，让用户一眼看懂盘面大势。
4. 【用神与关键依据】明确本次主用神取什么、为何这样取、落在哪一宫；该宫的门、星、神是帮它还是压它。${hasPersonal ? '须说明本人/伴侣/生年干符号落宫与状态（击刑/入墓/空亡），直接引用预计算事实。' : ''}
